...
function WaitLote($loteId, [string]$statusDesejado = "concluido", [int]$maxTentativas = 20) {
    for ($i = 1; $i -le $maxTentativas; $i++) {
        ...
        Log "  tentativa $i — status: $($lote.status)"
        if ($lote.status -eq $statusDesejado -or $lote.status -eq "pendente" -or $lote.status -eq "erro") {
            return $lote
        }
    }
...
Log "Status apos processamento: $($lote.status)"

if ($lote.status -eq "erro") {
    Fail "Lote com erro: $($lote.mensagem_erro)"
}

# ── Passo 6: Resolver Pendencias (se houver) ─────────────────
if ($lote.status -eq "pendente") {
    Log ""
    Log "PASSO 6 — Resolver Pendencias"

    $pendencias = (CurlGet "/api/v1/lotes/$loteId").pendencias
    $contas = $pendencias | Where-Object { -not $_.resolvida } | Select-Object -ExpandProperty conta_cliente
    Log "Pendencias encontradas: $($contas -join ', ')"

    # Mapear: 1130->25 (unica pendencia esperada apos fix E3)
    # Caso ainda apareçam 18 e 22 (test-parse ainda nao corrigido), mapear como passthrough
    $mapeamentos = @{}
    foreach ($conta in $contas) {
        switch ($conta) {
            ...
            default {
                Log "  AVISO: conta inesperada como pendente: $conta — mapeando para ela mesma"
    ...
    Log "Status final: $($lote.status)"
}

if ($lote.status -ne "concluido") {
    Fail "Lote nao concluido — status: $($lote.status) | erro: $($lote.mensagem_erro)"
}
...
# Normalizar quebras de linha para comparacao
